...
namespace OOP_7
{
    ...
    {
        public delegate void AddBook(string message, string heading);

        public delegate void RemoveBook(string message, string heading);

        public static int QuantityLibraries;
        ...
        public string Librarian { get; }
        public int NumberOfBooks { get; private set; }
        public int NumberOfHalls { get; }
        public int NumberOfSeatsInTheHall { get; }
        public float VisitorsPerDay { get; }
        public event AddBook? Add;
        public event RemoveBook? Remove;
        ...
        public override string ToString()
        {
            return $"Название: {Name}" + Environment.NewLine +
                   ...
                   $"Среднее количество посетителей в день: {VisitorsPerDay}" + Environment.NewLine +
                   Environment.NewLine;
        }
        ...
        public static Library operator +(Library lib, Book book)
        {
                                    ...
                                    && b.Publisher == book.Publisher))
            {
                ...
                lib.Add?.Invoke("Книга успешно добавлена в библиотеку", "Юху!");
            }
            else
            {
                MessageBox.Show(
        ...
        public static Library operator -(Library lib, Book book)
        {
            if (lib.Books.Remove(book)) lib.Remove?.Invoke("Книга успешно удалена из библиотеки", "Жаль:(");
            lib.NumberOfBooks = lib.Books.Count;
            return lib;
        }
    }
